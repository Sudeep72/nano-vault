...
async def test_cross_user_isolation(client: AsyncClient, auth_headers):
    ...
    bob_name = f"bob_{_uuid.uuid4().hex[:6]}"
    await client.post("/api/v1/auth/register", json={
        "username": bob_name, "email": f"{bob_name}@example.com", "password": "BobsPass1!"
    })
    login = await client.post("/api/v1/auth/login", json={"username": bob_name, "password": "BobsPass1!"})
